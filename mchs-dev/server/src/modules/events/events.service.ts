...
export class EventsService {
  constructor(
    @InjectRepository(SEvents, 'mchs_connection')
    private eventsRepository: Repository<SEvents>,
    @InjectRepository(SEventsOrder, 'mchs_connection')
    private eventsOrderRepository: Repository<SEventsOrder>,
    @InjectRepository(SEventsQue, 'mchs_connection')
    private eventsQueRepository: Repository<SEventsQue>,
    @InjectRepository(SEventsDef, 'mchs_connection')
    private eventsDefRepository: Repository<SEventsDef>,
    @InjectRepository(SEventsOrderAdmForce, 'mchs_connection')
    private eventsOrderAdmForceRepository: Repository<SEventsOrderAdmForce>,
    @InjectRepository(SEventsOrderAdmBan, 'mchs_connection')
    private eventsOrderAdmBanRepository: Repository<SEventsOrderAdmBan>,
    @InjectRepository(SEventsOrderQueDef, 'mchs_connection')
    private eventsOrderQueDefRepository: Repository<SEventsOrderQueDef>,
    @InjectRepository(SEventsPrivate, 'mchs_connection')
    private eventsPrivateRepository: Repository<SEventsPrivate>,
    @InjectRepository(SEventsOrderDefMtx, 'mchs_connection')
    private eventsOrderDefMtx: Repository<SEventsOrderDefMtx>,
  ...
  async createE(eventDto: CreateEventDTO) {
    ...
    if ((await this.isExistedEvent(event)) === true) {
      throw new EventBadRequestException(
        `Мероприятие id = ${event.idEvent} уже создано`,
      );
    } else {
      return this.eventsRepository.save(event);
    }
  ...
  async isExistedEvent(eventDto: CreateEventDTO) {
    const exists = await this.eventsRepository.find({
      where: {
  ...
  async getAllEventsPrivateSortAndPage(
    field: string,
    order: string,
    current: string,
    pageSize: string,
  ...
  async getAllEventsOrdersWithRelationsSortAndPage(
    field: string,
    order: string,
    current: string,
    pageSize: string,
    ...
    const events = await this.eventsOrderRepository.find({
      where: {
        active: 1,
      },
      relations: {
  ...
  async getAllEventsOrderQueDefsWithRelationsSortAndPage(
    field: string,
    order: string,
    current: string,
    pageSize: string,
    ...
    const events = await this.eventsOrderQueDefRepository.find({
      where: {
        active: 1,
      },
      relations: {
  ...
  async getAllEventDefs() {
    const events = await this.eventsDefRepository.find({
      where: {
        active: 1,
      },
    });
  ...
  async getAllEventQues() {
    const events = await this.eventsQueRepository.find({
      where: {
        active: 1,
      },
    });
  ...
  async getAllEventOrderAdmForces() {
    const events = await this.eventsOrderAdmForceRepository.find({
      where: {
        active: 1,
      },
    });
  ...
  async getAllEventOrderAdmBans() {
    const events = await this.eventsOrderAdmBanRepository.find({
      where: {
        active: 1,
      },
    });
  ...
  async getAllDayEvents(dateNow: GetNowDTO): Promise<SEvents[]> {
    ...
    if (dayEvents.length == 0) {
      throw new EventNotFoundException(
        `Мероприятий на ${dayEvents} не запланировано.`,
      ); // проверить в каком виде даты он выведет dayEvents
  ...
  async getAllWeekEvents(dateNow: GetNowDTO): Promise<SEvents[]> {
    ...
    if (weekEvents.length == 0) {
      throw new EventNotFoundException(
        'На этой неделе нет спланированных мероприятий.',
      ); // no events this week, мб если можно то написать так: Нет спланированных мероприятий с ${dateBegin} по ${dateEnd}(просто разбить, и в Between 2 переменные)
    }
    return weekEvents;
  }
  ...
  async getAllMonthEvents(dateNow: GetNowDTO): Promise<SEvents[]> {
    ...
    if (monthEvents.length == 0) {
      throw new EventNotFoundException(
        'В этом месяце нет спланированных мероприятий.',
      ); // no month events
    }
    return monthEvents;
  }
  ...
  async getEventPrivateById(idPriv: number): Promise<SEventsPrivate> {
    ...
    if (!event) {
      throw new EventNotFoundException(
        `Event Private id = ${idPriv} not found!`,
      );
    }
    return event;
  }
  ...
  async getEventOrderAdmForceById(idList: number) {
    const event = await this.eventsOrderAdmForceRepository.findOneBy({
      idList,
    });
    if (!event) {
      throw new EventNotFoundException(
        `Event Order Adm Force id = ${idList} not found!`,
      );
    }
    return event;
  }
  ...
  async getEventOrderAdmBanById(idList: number) {
    ...
    if (!event) {
      throw new EventNotFoundException(
        `Event Order Adm Ban id = ${idList} not found!`,
      );
    }
    return event;
  }
  ...
  async getEventBeginDateById(idEvent: number) {
    const eventBeginDate = (await this.eventsRepository.findOneBy({ idEvent }))
      .dateBegin;
    return eventBeginDate;
  }

  async getEventEndDateById(idEvent: number) {
    const eventEndDate = (await this.eventsRepository.findOneBy({ idEvent }))
      .dateEnd;
    return eventEndDate;
  }
  ...
  async getEventOrderQueDefById(idList: number) {
    ...
    if (!event) {
      throw new EventNotFoundException(
        `Event Order Que Def id = ${idList} not found!`,
      );
    }
    return event;
  }
  ...
  async updateEventOrderAdmForce(
    idList: number,
    dto: CreateEventOrderAdmForceDTO,
  ) {
  ...
  async deleteEventOrderQueDefById(idList: number) {
    const result = await this.eventsOrderQueDefRepository.update(idList, {
      active: 0,
    });
    return result;
  }
  ...
  async deleteEventOrderAdmForceById(idList: number) {
    const result = await this.eventsOrderAdmForceRepository.update(idList, {
      active: 2,
    });
    return result;
  }
  ...
  async deleteEventOrderAdmBanById(idList: number) {
    const result = await this.eventsOrderAdmBanRepository.update(idList, {
      active: 2,
    });
    return result;
  }
