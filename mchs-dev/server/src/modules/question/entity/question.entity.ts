import { SEventsOrderQue } from "src/modules/events/entity/eventsOrderQue.entity";
import { SEventsQue } from "src/modules/events/entity/eventsQue.entity";
...
export class SQuestion {
  ...
  @Column("bigint", {
    ...
    nullable: true,
    comment:
      "Раздел ВОПРОСЫ ПРОВЕРКИ МТХ (МОНИТОРИНГА) (doc.s_units.type_unit=40 , брать из name)",
    unsigned: true,
  })
  ...
  @OneToMany(() => SEventsOrderQue, (sEventsOrderQue) => sEventsOrderQue.idQue2)
  sEventsOrderQues: SEventsOrderQue[];

  @OneToMany(() => SEventsQue, (sEventsQue) => sEventsQue.idQue2)
