import { Column, Entity, PrimaryGeneratedColumn } from "typeorm";

@Entity("s_oon", { schema: "doc" })
export class SOon {
  @PrimaryGeneratedColumn({ type: "int", name: "id_type", unsigned: true })
  idType: number;

  @Column("bigint", { name: "id_parent", nullable: true, unsigned: true })
  idParent: string | null;

  @Column("varchar", {
    name: "num_",
    nullable: true,
    comment: "Номер (ООН опасных грузов чл_4)",
    length: 55,
  })
  num: string | null;

  @Column("varchar", {
    name: "type",
    nullable: true,
    comment: "Тип опасности (Класс опасных грузов)",
    length: 60,
  })
  type: string | null;

  @Column("varchar", {
    name: "type_sub",
    nullable: true,
    comment: "Подкласс опасных грузов (чл_4)",
    length: 21,
  })
  typeSub: string | null;

  @Column("varchar", {
    name: "name",
    nullable: true,
    comment: "Описание опасности",
    length: 550,
  })
  name: string | null;

  @Column("date", {
    name: "date_record",
    nullable: true,
    comment: "Дата изменения записи",
    default: () => "'now()'",
  })
  dateRecord: string | null;

  @Column("tinyint", {
    name: "active",
    comment: "0-удалено, 1-актино",
    unsigned: true,
    default: () => "'1'",
  })
  active: number;

  @Column("int", {
    name: "uid",
    nullable: true,
    comment: "Пользователь, внесший изменения",
    unsigned: true,
  })
  uid: number | null;
}
